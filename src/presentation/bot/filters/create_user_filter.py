...
class CreateUserFilter(Filter):
    ...
    async def __call__(
        ...
        data = {"user": user}
        if isinstance(message, Message) and (message.sender_chat is None or message.sender_chat.type in [
            ...
            user_id = await create_user(data=CreateUserDtoInput(
                ...
                language=Language(
                    Language.ru if event_from_user.language_code is None else
        ...
        elif user.member == Member.administrator:
            data["content"] = content(language=Language.ru)
        else:
