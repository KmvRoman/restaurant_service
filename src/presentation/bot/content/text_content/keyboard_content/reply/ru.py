...
from src.presentation.bot.content.text_content.keyboard_content.reply.enums import (
    MainMenuRu, HomeRu, AdminMainMenuRu, AdminHomeRu, BackRu, ChooseOrderTypeRu, SendLocationRu, WebAppRu,
    MyPhoneNumberRu, SkipRu, AcceptRu, PaymentTypesUz,
)
...
class RussianReplyKeyboardText(IReplyKeyboardText):
    ...
    def users_main_menu(self) -> ReplyKeyboardMarkup:
        return self.reply_keyboard.users_main_menu(buttons=MainMenuRu)

    def main_menu_button(self) -> ReplyKeyboardMarkup:
        return self.reply_keyboard.main_menu_button(buttons=HomeRu)

    def back_button(self) -> ReplyKeyboardMarkup:
        return self.reply_keyboard.back_button(button=BackRu)

    def administrators_main_menu(self) -> ReplyKeyboardMarkup:
        return self.reply_keyboard.admins_main_menu(buttons=AdminMainMenuRu)

    def administrator_home(self) -> ReplyKeyboardMarkup:
        return self.reply_keyboard.administrator_home(buttons=AdminHomeRu)

    def choose_order_type_keyboard(self) -> ReplyKeyboardMarkup:
        keyboard = self.reply_keyboard.choose_order_type(buttons=ChooseOrderTypeRu)
    ...
    def send_location_shipping(self, payload: GetUserAddressesDtoOutput) -> ReplyKeyboardMarkup:
        ...
        buttons = [SendLocationRu.location]
    ...
    def webapp_input_keyboard(
            ...
            user_location: Location, language: Language, restaurant_location_id: int,
            order_type: OrderType,
    ) -> ReplyKeyboardMarkup:
        keyboard = self.reply_keyboard.webapp_menu(
            button=WebAppRu, user_id=user_id, restaurant_id=restaurant_id,
    ...
    def user_comment_keyboard(self) -> ReplyKeyboardMarkup:
        keyboard = self.reply_keyboard.skip_button(button=SkipRu)
        keyboard.keyboard.extend(self.back_button().keyboard)
        return keyboard

    def user_phone_number_keyboard(self) -> ReplyKeyboardMarkup:
        keyboard = self.reply_keyboard.phone_number(button=MyPhoneNumberRu)
        keyboard.keyboard.extend(self.back_button().keyboard)
        return keyboard

    def user_accept_order_keyboard(self) -> ReplyKeyboardMarkup:
        keyboard = self.reply_keyboard.accept_order(button=AcceptRu)
        keyboard.keyboard.extend(self.back_button().keyboard)
        return keyboard
